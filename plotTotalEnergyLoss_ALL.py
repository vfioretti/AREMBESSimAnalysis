"""
 plotTotalEnergyLoss_ALL.py  -  description
 ---------------------------------------------------------------------------------
 ...
 ----------------------------------------------
 Usage:
 python plotTotalEnergyLoss_ALL.py N_in energy_0 rebin mscatt_flag
 ---------------------------------------------------------------------------------
 Parameters:
...
mscatt_flag = int(arg_list[4])

# experimental data
# 1MeV
N_exp = 6
if mscatt_flag == 1: 
  color_exp = ['blue', 'purple', 'orange']
else: 
  color_exp = ['orange']
...
if mscatt_flag == 1: 
   models = ['MultipleScattering', 'MultipleScattering_Option4', 'SingleScattering'] 
   models_title = ['Multiple Scattering (option 3)', 'Multiple Scattering (option 4)', 'Single Scattering']
   model_fmt = ['-s', '-<', '->']
else:
   models = ['SingleScattering'] 
   models_title = ['Single Scattering']
   model_fmt = ['->']
...
for jexp in xrange(N_exp):
 for jm in xrange(len(models)):
    ...
    name_file_in = './diebold_test_results/'+str(int(energy_0))+'keV_'+str(theta_0_array[jexp])+'_'+str(N_in)+'/'+models[jm]+'_energy.dat'
    ...
    ax2.errorbar(theta_xrange_rebin, mean_energy_loss_rebin, xerr=err_theta_xrange_rebin, yerr=err_mean_energy_loss_rebin, capsize=0, fmt=model_fmt[jm], ms=10, lw = 2, ecolor=color_exp[jm], color=color_exp[jm], label='BoGEMMS - '+models_title[jm])
 ...
 title = str(N_in)+" events, "+str(energy_0)+" keV, "+str(theta_0_array[jexp])+" deg."
 ax2.set_title(title)
 ax2.set_yscale('log')
